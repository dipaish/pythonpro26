...
This project’s website is built with MkDocs (Dracula theme) and deployed using GitHub Pages “Deploy from a branch”.

- Branch used for deploy: `mkdocsTesting`
- Folder: `/site` (generated by MkDocs)
- Trigger: any push to `mkdocsTesting` that changes `docs/**` or `mkdocs.yml`
- GitHub Pages setting: Source = “Deploy from a branch” → Branch: `mkdocsTesting` / Folder: `/site`

How it works:

1. A GitHub Actions workflow builds the docs with the Dracula theme and writes the output into `site/`.
2. The workflow commits `site/` back to `mkdocsTesting` (with “[skip ci]” to avoid loops).
3. GitHub Pages serves the content from `mkdocsTesting` → `/site`.

How to publish updates:

1. Commit your documentation changes (files in `docs/` or `mkdocs.yml`) to `mkdocsTesting`.
2. Push to GitHub. The workflow “Build MkDocs to site (Dracula)” will run and update `site/`.
3. Visit https://dipaish.github.io/pythonpro26/ (hard refresh with Ctrl+F5 if needed).

Notes:

- If you prefer deploying via “GitHub Actions” instead, switch Pages Source accordingly and replace the workflow with a Pages deploy job.
- If the site doesn’t look themed, confirm you’re serving from `/site` and that the latest workflow run succeeded.
